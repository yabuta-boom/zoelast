...
import { Plus, Eye, Edit, Trash2, Filter, Download, MoreHorizontal, FileText, Share2, Archive, MessageCircle } from 'lucide-react';
...
interface Vehicle {
  ...
  isTradeIn: boolean;
  userId?: string; // Add userId to track the owner
}
...
const VehicleDetailsModal: React.FC<VehicleDetailsModalProps> = ({ vehicle, onClose, onChatWithUser }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-lg font-semibold">Trade-In Vehicle Details</h2>
          <button 
            onClick={onClose}
        ...
        <div className="flex justify-end gap-2 p-4 border-t">
          ...
          {vehicle.userId && (
            <button
              onClick={() => onChatWithUser(vehicle.userId!)}
...
const TradeIn: React.FC = () => {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      const vehicleList = querySnapshot.docs
        .map(doc => ({
          id: doc.id,
          ...doc.data(),
        ...
        .filter(vehicle => vehicle.isTradeIn) as Vehicle[];
      setVehicles(vehicleList);
    } catch (error) {
      console.error('Error fetching trade-in vehicles:', error);
      setError('Failed to load trade-in vehicles');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleChatWithUser = (userId: string) => {
    ...
    navigate('/admin?tab=inbox', { state: { selectedUserId: userId } });
  };
  ...
  const handleExport = () => {
    ...
    utils.book_append_sheet(wb, ws, 'Trade-In Vehicles');

    writeFile(wb, `trade-in-export-${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleGenerateReport = () => {
    ...
    const ws = utils.json_to_sheet([{
      'Total Trade-In Vehicles': reportData.totalVehicles,
    ...
    const wb = utils.book_new();
    utils.book_append_sheet(wb, ws, 'Trade-In Report');

    writeFile(wb, `trade-in-report-${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6 pt-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold">Trade-In Management</h2>
          <p className="text-gray-600">Manage your trade-in vehicle inventory</p>
        </div>
                ...
                <tr key={vehicle.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center space-x-3">
                      ...
                      {vehicle.userId && (
                        <button
                          onClick={() => handleChatWithUser(vehicle.userId!)}
          ...
          <div className="bg-white rounded-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Add New Trade-In Vehicle</h2>
                <button
      ...
      {showEditForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">Edit Trade-In Vehicle</h2>
                <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm mx-4">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete this trade-in vehicle? This action cannot be undone.
            </p>
