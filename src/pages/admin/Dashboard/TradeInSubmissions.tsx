...
const TradeInSubmissions: React.FC = () => {
  ...
  // Filter only trade-in submissions
  const filteredMessages = messages.filter(
    (message) =>
      message.type === 'car' &&
      (message as CarSubmission).submissionType === 'trade-in' &&
      (searchQuery === '' ||
  ...
  const handleDeleteMessage = (message: Message) => {
    if (window.confirm('Are you sure you want to delete this trade-in submission?')) {
  ...
  const handleAddToInventory = async (carSubmission: CarSubmission) => {
    try {
      // Create vehicle document in Firestore with trade-in flag
      const vehicleData = {
        ...
        condition: 'used',
        sold: false,
        isTradeIn: true, // Mark as trade-in vehicle
        createdAt: new Date()
      };

      await addDoc(collection(db, 'vehicles'), vehicleData);
      
      // Close the modal and navigate to trade-in inventory
      setSelectedMessage(null);
      navigate('/admin?tab=trade-in');
    } catch (error) {
      console.error('Error adding to trade-in inventory:', error);
      alert('Failed to add vehicle to trade-in inventory. Please try again.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Review and manage trade-in vehicle submissions</p>
      </div>
      ...
      {filteredMessages.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No Trade-In Submissions</h3>
          <p className="text-gray-600">No trade-in submissions have been received yet.</p>
        </div>
      ) : (
        <DataTable
          ...
          columns={[
            {
              ...
              render: () => (
                <div className="flex items-center">
                  ...
                  <span className="ml-2">Trade-In</span>
                </div>
              ),
            },
            {
...
export default TradeInSubmissions;